...
from idraw2_0internal.idraw_options import common_options
...
class iDrawWrapperClass( inkex.Effect ):
    """ Main wrapper class for operating multiple iDraw units """
    ...
    def __init__( self, default_logging = True, params = None ):
        if params is None:
            ...
            params = import_module("idraw2_0internal.idraw_conf") # Configuration file
        self.params = params
        self.status_code = 0
    ...
    def set_up_pause_transmitter(self):
        ...
        # one pause event for all idraws
    ...
    def effect( self ):
        ...
        USB port use option (self.options.port_config)
            ...
            0: Default behavior:
                ...
                * If no port is specified, use the first available iDraw

            1: Use first iDraw located via USB, even if a port is given.
    
            2: Use only specified port, given by self.options.port

            3: Plot to all attached iDraw units
        '''
        ...
        if self.options.port_config == 3: # Use all available iDraw units.
            process_list = []
            drawcore_list = []
            drawcore_list = drawcore_serial.listDRAWCOREports()

            if drawcore_list:
                primary_port = None
                if self.options.port is not None:
                    primary_port = drawcore_serial.find_named_drawcore(self.options.port)

                for found_port in drawcore_list:
                    logger.info("Found an DrawCore:")
                ...
                if len(drawcore_list) == 1:
                    logger.info("Found a single iDraw2 via USB.")
                    self.plot_to_idraw(None, True)
                else:
                    if primary_port is None:
                        primary_port = drawcore_list[0][0]
                    for index, found_port in enumerate(drawcore_list):
                        ...
                        if USE_MULTIPROCESSING:
                            process = multiprocessing.Process(target=self.plot_to_idraw,
                                args=(found_port[0],False))
                        else: # Use multithreading:
                            tname = "thread-" + str(index)
                            process = threading.Thread(group=None, target=self.plot_to_idraw,
                    ...
                    self.plot_to_idraw(primary_port, True) # Plot to "primary" iDraw
                    for process in process_list:
                        logger.info("Joining a process. ")
                        process.join()
            else: # i.e., if not drawcore_list
                logger.error("No available idraw2 units found on USB.")
                logger.error("Please check your connection(s) and try again.")
                return
        else:   # All cases except plotting to all available idraw units:
                # This includes: Preview mode and all cases of plotting to a single idraw.

            # If we are to use first available unit, blank the "port" variable.
            if self.options.port_config == 1: # Use first available idraw
                self.options.port = None
            self.plot_to_idraw(self.options.port, True)

    def plot_to_idraw( self, port, primary):
        """ Delegate the plot to a particular idraw """
#         if primary:
#             pass
#         else:
...
#             return # Skip secondary units, without opening class or serial connection
        ...
        ad = idraw.iDraw(params=self.params, default_logging=self.default_logging)
        ad.set_up_pause_receiver(self.software_initiated_pause_event)
        ad.getoptions([])

        prim = "primary" if primary else "secondary"
        logger.info("plot_to_idraw started, at port %s (%s)", port, prim)
        ...
        selected_options = {item: self.options.__dict__[item] for item in ['mode',
            'speed_pendown', 'speed_penup',  'accel', 'pen_pos_up', 'pen_pos_down', 'laser_power', 'laser_focus_power',
        ...
        # to be "use first available idraw":
        if port is None:
            ad.options.port_config = 1 # Use first available idraw
        else:
            ad.options.port_config = 2 # Use idraw specified by port
        ...
        ad.effect() # Plot the document using idraw.py

        if primary:
            self.document = ad.document
            self.outdoc =  ad.get_output() # Collect output from idraw.py
            self.status_code = ad.plot_status.stopped
        else:
            if ad.error_out:
                if port is not None:
                    logger.error('Error on iDraw2 at port "' + port + '":' + ad.error_out)
                else:
                    logger.error('Error on secondary iDraw2: ' + ad.error_out)
