...
from idraw2_0internal.idraw_options import common_options, versions

from idraw2_0internal import path_objects
from idraw2_0internal import digest_svg
from idraw2_0internal import boundsclip
from idraw2_0internal import plot_optimizations
from idraw2_0internal import plot_status
from idraw2_0internal import pen_handling
from idraw2_0internal import plot_warnings
from idraw2_0internal import serial_utils
from idraw2_0internal import motion
from idraw2_0internal import dripfeed
from idraw2_0internal import preview
...
class iDraw(inkex.Effect):
    """ Main class for idraw """
    ...
    def __init__(self, default_logging=True, user_message_fun=message.emit, params=None):
        if params is None:
            params = import_module("idraw2_0internal.idraw2_0_conf") # Default configuration file
        self.params = params
    ...
    def set_up_pause_receiver(self, software_pause_event):
        ...
        keyboard interrupt (ctrl-C) to pause the idraw """
    ...
    def set_secondary(self, suppress_standard_out=True):
        """ If a "secondary" idraw called by idraw_control """
    ...
    def update_options(self):
        ...
        # Physical travel bounds, based on idraw model:
        ...
        elif self.options.model == 8:
            self.x_bounds_max = self.params.x_travel_SEA0
            self.y_bounds_max = self.params.y_travel_SEA0
        else:
        ...
        self.options.pen_pos_up = plot_utils.constrainLimits(self.options.pen_pos_up, 0, 10)
        self.options.pen_pos_down = plot_utils.constrainLimits(self.options.pen_pos_down, 0, 10)
        self.options.pen_rate_raise = plot_utils.constrainLimits(self.options.pen_rate_raise, 1, 15000)
        self.options.pen_rate_lower = plot_utils.constrainLimits(self.options.pen_rate_lower, 1, 15000)
        self.options.speed_pendown = plot_utils.constrainLimits(self.options.speed_pendown, 1, 15000)
        self.options.speed_penup = plot_utils.constrainLimits(self.options.speed_penup, 1, 15000)
    ...
    def effect(self):
        ...
        if self.options.mode == "manual":
            ...
            if self.options.manual_cmd == "strip_data":
                ...
                self.user_message_fun(gettext.gettext(\
                    "All iDraw data has been removed from this SVG file."))
                return
            ...
            if self.options.manual_cmd == "list_names": # Run before regular serial connection!
                self.name_list = drawcore_serial.list_named_drawcores() # Variable available for python API
                if not self.name_list:
                    self.user_message_fun(gettext.gettext("No named iDraw2 units located.\n"))
                else:
                    self.user_message_fun(gettext.gettext("List of attached iDraw2 units:"))
                    for detected_drawcore in self.name_list:
                        self.user_message_fun(detected_drawcore)
                return
            if self.options.manual_cmd == "list_port_info":
                ports = drawcore_serial.list_port_info() # Variable available for python API
                for port in ports:
                    logger.error(port)
                return
        ...
        if self.plot_status.port is None and not self.options.preview:
            return # unable to connect to idraw

        if self.options.mode in ('align', 'toggle', 'cycle', 'raise_pen', 'lower_pen', 'laser_on', 'laser_off','laser_focus','cycle_laser'):
            self.setup_command()
        ...
        if self.options.mode in ("plot", "layers", "res_plot"):
            ...
            if (math.fabs(self.pen.phys.xpos < 0.001) and
                ...
                logger.error(gettext.gettext(\
                    "Unable to move to Home. (Is the iDraw already at Home?)"))
                return

            # self.query_drawcore_voltage()
        ...
        if self.plot_status.port is not None:
            drawcore_motion.doTimedPause(self.plot_status.port, 10, False) # Final timed motion command
    ...
    def setup_command(self):
        ...
        if self.options.mode == "align":
            self.pen.pen_raise(self)
            drawcore_motion.sendDisableMotors(self.plot_status.port, False)
        elif self.options.mode == "cycle":
            self.pen.cycle(self)
        elif self.options.mode == "raise_pen":
            self.pen.pen_raise(self)
        elif self.options.mode == "lower_pen":
            self.pen.pen_lower(self)
        elif self.options.mode == "laser_focus":
            drawcore_motion.PBOutValue( self.plot_status.port,1000, int(self.options.laser_focus_power*10)) # I/O Pin PWM output: high
        elif self.options.mode == "laser_on":
            drawcore_motion.PBOutValue( self.plot_status.port,1000, int(self.options.laser_power*10)) # I/O Pin PWM output: high
        elif self.options.mode == "laser_off":
            drawcore_motion.PBOutValue( self.plot_status.port,1000, 0) # I/O Pin PWM output: high
        elif self.options.mode == "cycle_laser":
            drawcore_motion.PBOutValue( self.plot_status.port,1000, int(self.options.laser_power*10)) # I/O Pin PWM output: high
            drawcore_serial.command(self.plot_status.port, 'G4 P0.5\r')
            drawcore_motion.PBOutValue( self.plot_status.port,1000, 0) # I/O Pin PWM output: high
    ...
    def manual_command(self):
        ...
        #     success = drawcore_serial.bootload(self.plot_status.port)
        #     if success:
        #         self.user_message_fun(
        #             gettext.gettext("Entering bootloader mode for firmware programming.\n" +
        #                             "To resume normal operation, you will need to first\n" +
        #                             "disconnect the iDraw from both USB and power."))
        #         self.disconnect() # Disconnect from iDraw; end serial session
        #     else:
        #         logger.error('Failed while trying to enter bootloader.')
        #     return

        if self.options.manual_cmd == "read_name":
            name_string = drawcore_serial.query_nickname(self.plot_status.port)
        ...
        if (self.options.manual_cmd).startswith("write_name"):
            ...
            if versions.min_fw_version(self.plot_status, "2.13"):
                renamed = drawcore_serial.write_nickname(self.plot_status.port, temp_string)
                if renamed is True:
                    if temp_string == "":
                ...
                # drawcore_serial.reboot(self.plot_status.port)    # Reboot required after writing nickname
                # self.disconnect() # Disconnect from iDraw; end serial session
            else:
                logger.error("This function requires a newer firmware version. ")
            return

        # self.query_drawcore_voltage() # Next: Commands that also require both power to move motors:
        if self.options.manual_cmd == "raise_pen":
            ...
            drawcore_motion.sendDisableMotors(self.plot_status.port, False)
        elif self.options.manual_cmd == "lock_motor":
            drawcore_serial.command(self.plot_status.port,'$1=255\r')
            self.user_message_fun("Motor locked.")
        elif self.options.manual_cmd == "unlock_motor":
            drawcore_serial.command(self.plot_status.port,'$1=254\r')
            self.user_message_fun("Motor unlocked.")   
        elif self.options.manual_cmd == "machine_origin":
            x_move_mm = self.y_bounds_max*25.4
            if self.options.model == 1:
                x_move_mm = x_move_mm-5
            drawcore_motion.GoHome(self.plot_status.port)
            drawcore_motion.doMove(self.plot_status.port,x_move_mm,0,5000)
            drawcore_serial.command(self.plot_status.port,'$SLP\r')
        elif self.options.manual_cmd == "list_drawcore_info":
            res = drawcore_serial.query_all(self.plot_status.port,'$$\r')
            logger.error(res)
        elif self.options.manual_cmd == "default_settings":
            res = drawcore_serial.query_all(self.plot_status.port,'$RST=*\r')
            logger.error(res)
            ...
            if self.options.manual_cmd == "walk_home":
                ...
                #     a_pos, b_pos = drawcore_motion.query_steps(self.plot_status.port, False)
                ...
                #         n_delta_y *= 2
                # else:
                #     logger.error("This function requires newer firmware. ")
                #     return
                return
            ...
            drawcore_serial.command(self.plot_status.port,'G1 F'+ str(self.speed_penup)+'\r')
            # self.go_to_position(f_x, f_y, ignore_limits=True)
            self.go_to_position(-f_y, -f_x, ignore_limits=True)
    ...
    def prepare_document(self):
        ...
        if valid_plob:
            ...
            if self.options.hiding:
                ...
                # when necessary
                from idraw2_0internal.clipping import ClipPathsProcess
    ...
    def pause_check(self):
        ...
        if pause_button_pressed == -1:
            self.user_message_fun('\nError: USB connection to iDraw2 lost. ' +\
        ...
        if pause_button_pressed == 1:
            if self.plot_status.delay_between_copies:
                ...
                logger.warning('Manually home this iDraw before plotting next item.\n')
    ...
    def enable_motors(self):
        ...
        if self.options.resolution == 1:  # High-resolution ("Super") mode
            if not self.options.preview:
                res_1, res_2 = drawcore_motion.query_enable_motors(self.plot_status.port, False)
                if not (res_1 == 1 and res_2 == 1): # Do not re-enable if already enabled
                    drawcore_motion.sendEnableMotors(self.plot_status.port, 1)  # 16X microstepping
            ...
            if not self.options.preview:
                ...
                if not (res_1 == 2 and res_2 == 2): # Do not re-enable if already enabled
                    drawcore_motion.sendEnableMotors(self.plot_status.port, 2)  # 8X microstepping
    ...
    # def query_drawcore_voltage(self):
    ...
    def disconnect(self):
        '''End serial session; disconnect from iDraw '''
        if self.plot_status.port:
            drawcore_serial.closePort(self.plot_status.port)
        self.plot_status.port = None
        self.connected = False  # Python interactive API variable

class SecondaryLoggingHandler(logging.Handler):
    '''To be used for logging to iDraw.text_out and iDraw.error_out.'''
    def __init__(self, idraw, log_name, level = logging.NOTSET):
        super().__init__(level=level)

        log = getattr(idraw, log_name) if hasattr(idraw, log_name) else ""
        setattr(idraw, log_name, log)
    ...
    def emit(self, record):
        assert(hasattr(self.idraw, self.log_name))
        new_log = getattr(self.idraw, self.log_name) + "\n" + self.format(record)
        setattr(self.idraw, self.log_name, new_log)

class SecondaryErrorHandler(SecondaryLoggingHandler):
    '''Handle logging for "secondary" machines, plotting alongside primary.'''
    def __init__(self, idraw):
        super().__init__(idraw, 'error_out', logging.ERROR)
