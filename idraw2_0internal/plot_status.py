...
Classes for managing iDraw plot status
...
class ResumeStatus:
    ...
    def write_to_svg(self, svg_tree):
        ...
        if not self.written:
            ...
            if self.new.application == "":
                self.new.application = "idraw"  # Name of this program
    ...
    def check_button(self, ad_ref):
        ...
        if (time_now - self.button_timestamp) > ad_ref.params.button_interval:
            ...
            str_button = drawcore_motion.QueryPRGButton(ad_ref.plot_status.port, False)
        else:
            return 0

        try:
            pause_button_pressed = int(str_button[0])
        except (IndexError, ValueError): # Generally indicates loss of connection to iDraw
            return -1
    ...
    def clear_button(self, ad_ref):
        ...
        drawcore_motion.QueryPRGButton(ad_ref.plot_status.port, False)
